<template>
  <v-container>
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title>
          <span class="headline">{{ selectedCompany ? 'Editar Empresa' : 'Crear Empresa' }}</span>
        </v-card-title>
        <v-card-text>
          <v-form @submit.prevent="submitForm">

            <v-text-field v-model="newCompany.nit" label="Nit" :rules="[requiredRule('Nit'), numberdRule]"
              required></v-text-field>

            <v-text-field v-model="newCompany.name" label="Nombre" :rules="[requiredRule('Nombre')]"
              required></v-text-field>

            <v-text-field v-model="newCompany.address" label="Dirección" :rules="[requiredRule('Dirección')]"
              required></v-text-field>

            <v-text-field v-model="newCompany.email" label="Correo electrónico"
              :rules="[requiredRule('Correo electrónico'), emailRule]" required></v-text-field>

            <v-text-field v-model="newCompany.phone" label="Teléfono" :rules="[requiredRule('Teléfono'), numberdRule]"
              required></v-text-field>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="submitForm">{{ selectedCompany ? 'Actualizar' : 'Guardar' }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-card>
      <v-card-title>
        <v-row>
          ...
          <v-col md="6" sm="6" cols="12">
            <v-text-field v-model="search" append-icon="mdi-magnify" label="Buscar Empresa" single-line hide-details
...
export default {
  ...
  setup() {
    ...
    const filteredCompanies = computed(() => {
      ...
      return companyList
        .filter(company => company.name.toLowerCase().includes(search.value.toLowerCase()))
    ...
    const submitForm = async () => {
      if (!newCompany.nit || !newCompany.name || !newCompany.phone || !newCompany.address || !newCompany.email) {
        return;
      }
      if (selectedCompany.value) {
        await companyStore.updateCompany({ ...selectedCompany.value, ...newCompany });
      } else {
